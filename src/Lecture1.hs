...
subString start end = take len . drop start'
    where
        start' = max 0 start
        len    = end - start' + 1
...
lowerAndGreater n list = go list (0, 0)
    where
        ...
        go [] (g, l) = show n <> " is greater than " <> show g <> " elements and lower than " <> show l <> " elements"
        go (x:xs) (g, l)
            | n >  x    = go xs (g+1, l)
            | n <  x    = go xs (g, l+1)
            | otherwise = go xs (g, l)
...
-- >>> lowerAndGreater 3 [1]
-- "3 is greater than 1 elements and lower than 0 elements"
